...
const startExam = async (req, res) => {
  try {
    ...
    if (exam.eligibility?.batch && exam.eligibility.batch !== student.batch) {
      return res.status(403).json({
        success: false,
        message: "You are not eligible for this examination.",
      });
    ...
    const insertResult = await db.collection("exam").insertOne(examAttempt);
...
const syncExam = async (req, res) => {
  try {
    ...
    const question = questionSet.questions.find(
      (q) => Number(q.questionNo) === Number(questionNo),
    );
    ...
    if (!question.options || typeof question.options !== "object") {
      return res.status(500).json({
        success: false,
        message: `Options are missing for question ${questionNo}.`,
      });
    }

    const optionValues = Object.values(question.options).map((option) =>
      String(option).trim(),
    );

    const answerExistsInOptions = optionValues.some(
      (option) =>
        option.toLowerCase() === normalizedStudentAnswer.toLowerCase(),
    );

    if (!answerExistsInOptions) {
      return res.status(400).json({
        success: false,
        message: `Invalid answer '${normalizedStudentAnswer}' for question ${questionNo}.`,
      });
    }
