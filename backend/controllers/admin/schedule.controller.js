...
const scheduleExam = async (req, res) => {
  try {
    ...
    if (semesterNumber != "odd" && semesterNumber != "even") {
      return res.status(400).json({
        success: false,
    ...
    const duplicateQuery = {
      ...
      "eligibility.academicYear": academicYear,

      "eligibility.semester": semesterNumber,
    ...
    if (normalizedCIE) {
      duplicateQuery.cie = normalizedCIE;
    }
    ...
    const exam = {
      ...
      eligibility: {
        ...
        admissionNo: Array.isArray(admissionNo) ? admissionNo : [],
      },
