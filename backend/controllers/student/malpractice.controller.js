  ...
  try {
    ...
    if (exam.status === false) {
      return res.status(403).json({
        success: false,
        message: "Exam has already been submitted or closed.",
        examClosed: true,
      });
    }
    ...
    const violationNo = malpracticeCount + 1;
    ...
    await db.collection("malpractice").insertOne(malpracticeDocument);
    ...
    if (violationNo >= MALPRACTICE_LIMIT) {
      ...
      return res.status(403).json({
        ...
        message: "Malpractice limit exceeded. Examination has been closed.",
    ...
    return res.status(200).json({
      ...
      message: "Malpractice recorded successfully.",
    ...
    console.error("MALPRACTICE ERROR:", error);
