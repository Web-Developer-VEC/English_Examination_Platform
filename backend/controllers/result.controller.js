...
const generateExamReport = async (req, res) => {
    ...
    try {
        ...
        const cleanAcademicYear = academicYear && typeof academicYear === "string" && academicYear.trim() !== "" ? academicYear.trim() : null;
        ...
        const testColumns = scheduleTests.map((test, index) => {
            ...
            const label = testNumber !== Number.MAX_SAFE_INTEGER
                ? `Test-${testNumber}`
        ...
        const rows = studentRoster
            .map((student, index) => {
                ...
                const marksCells = testColumns
                    .map((col) => {
                        const entry = studentTests ? studentTests.get(col.questionSetId) : undefined;
                        let markDisplay = "AB";
        ...
        const semesterValue = cleanSem ? cleanSem.charAt(0).toUpperCase() + cleanSem.slice(1) : "All";
        ...
        const sanitizeForFilename = (value) => {
            return String(value).trim().replace(/[^a-zA-Z0-9-]+/g, "") || "All";
        };
