...
const getNextAcademicYear = (academicYears) => {
  ...
  const parsedYears = academicYears
    .map((item) => {
      const value =
        typeof item === "string" ? item : item?.year || item?.academicYear;
...
const StudentProfileAccess = () => {
  ...
  const [addingAcademicYear, setAddingAcademicYear] = useState(false);
  ...
  const fetchScheduleData = async () => {
    ...
    try {
      ...
      showMessage(
        getApiErrorMessage(error, "Unable to load filter data."),
        "error",
      );
    } finally {
  ...
  const batchOptions = useMemo(
    () => [
      ...new Set(
        batchDepartmentSections.map((item) => item.batch).filter(Boolean),
      ),
    ],
  ...
  const sectionOptions = useMemo(() => {
    ...
    return [...new Set(rows.map((item) => item.section).filter(Boolean))];
  }, [batchDepartmentSections, selectedBatch, selectedDepartment]);

  const fetchStudents = async ({ silent = false } = {}) => {
    if (!selectedBatch || !selectedDepartment || !selectedSection) {
      showMessage("Please select batch, department and section.", "error");
      return;
    }
    ...
    try {
      ...
      if (!silent && normalizedStudents.length === 0) {
        showMessage("No students found for the selected group.", "error");
      }
    } catch (error) {
      console.error("Student fetch error:", error);
      setStudents([]);
      showMessage(
        getApiErrorMessage(error, "Unable to load student data."),
        "error",
      );
    } finally {
  ...
  const handleProfileAccessToggle = async (student) => {
    ...
    try {
      ...
      showMessage(
        getApiErrorMessage(error, "Unable to update profile access."),
        "error",
      );
    } finally {
  ...
  const handleAllStudentsAccess = async (enabled) => {
    ...
    if (!permissionList.length) {
      showMessage("No students with admission numbers are available.", "error");
      return;
    }
    ...
    try {
      const data = await updateStudentProfileAccess(permissionList);
      ...
      showMessage(
        getApiErrorMessage(error, "Unable to update student permissions."),
        "error",
      );
    } finally {
      setUpdatingStudentId(null);
  ...
  const fetchAcademicYears = async () => {
    ...
    try {
      ...
      if (data?.success === false) {
        throw new Error(getErrorMessage(data, "Unable to load academic year."));
      }
      ...
      showMessage(
        getApiErrorMessage(error, "Unable to load academic year."),
        "error",
      );
    } finally {
  ...
  const handleAddAcademicYear = async () => {
    ...
    if (!/^\d{4}-\d{4}$/.test(value)) {
      showMessage("Academic year must be in YYYY-YYYY format.", "error");
      return;
    }
    ...
    if (endYear !== startYear + 1) {
      showMessage("Invalid academic year. Example: 2026-2027.", "error");
      return;
    }
    ...
    try {
      const data = await updateAcademicYear(value);

      if (data?.success === false) {
        throw new Error(
          getErrorMessage(data, "Unable to update academic year."),
        );
      }

      const updatedYear =
        data?.data?.academicYear || data?.academicYear || value;
      ...
      showMessage(
        getApiErrorMessage(error, "Unable to update academic year."),
        "error",
      );
    } finally {
      ...
      <div className="profile-access-container">
        <header className="profile-access-header">
          <div className="header-brand">
            ...
            <div>
              <h1>Student Profile Access</h1>

              <p>
                Control student Edit Profile access from one secure admin panel.
              </p>
            </div>
          </div>
        </header>
        ...
        <section className="access-card filter-card">
          <div className="section-heading">
            ...
            <div>
              <h2>Select Student Details</h2>

              <p>
                Choose batch, department and section to manage student profile
                access.
              </p>
            </div>
          </div>
          ...
          <div className="filter-grid">
            ...
            <div className="filter-field">
              ...
              <ThemeDropdown
                ...
                loading={loadingScheduleData}
                disabled={
                  !selectedBatch || !selectedDepartment || loadingScheduleData
                }
              />
            </div>
        ...
        <section className="access-card student-card">
          <div className="student-card-header">
            <div className="section-heading">
              ...
              <div>
                ...
                <p>Enable or disable Edit Profile for individual students.</p>
              </div>
            </div>
          ...
          {students.length > 0 ? (
                  ...
                  <tbody>
                    {paginatedStudents.length > 0 ? (
                      paginatedStudents.map((student, index) => {
                        const updating = updatingStudentId === student.id;
              ...
              {filteredStudents.length > 0 && (
                <div className="pagination">
                  <div className="pagination-info">
                    Showing{" "}
                    <strong>{(currentPage - 1) * STUDENTS_PER_PAGE + 1}</strong>
                    {" - "}
                    <strong>
                      {Math.min(
                        currentPage * STUDENTS_PER_PAGE,
            ...
            <div className="student-empty-state">
              ...
              <p>
                Select batch, department and section above, then click Manage
                Students.
              </p>
            </div>
          )}
          ...
          <div className="academic-header">
            <div className="section-heading">
              ...
              <div>
                ...
                <p>Update the current academic year used by the portal.</p>
              </div>
            </div>
          </div>
              ...
              <div className="academic-direct-edit">
                ...
                <div className="academic-direct-actions">
                  ...
                  <button
                    ...
                    disabled={addingAcademicYear || !newAcademicYear.trim()}
        ...
        <div className="security-footer">
          ...
          <span>
            Only administrators can change student profile access permissions.
          </span>
        </div>
      </div>
