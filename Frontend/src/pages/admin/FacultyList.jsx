...
const FacultyList = () => {
  ...
  const getGroupedAllowdept = (member) => {
    const flatAssignments = getFlattenedAllowdept(
      member?.allowdept || member?.assignments,
    );
    ...
    return Object.keys(grouped)
      .map((batch) => ({ batch, classes: grouped[batch] }))
  ...
  const fetchScheduleData = async () => {
    try {
      ...
      showPopup(
        "Error",
        getApiErrorMessage(error, "Unable to load department and section"),
      );
  ...
  const fetchStaff = async () => {
    try {
      ...
      const rows =
        result?.data?.staff || result?.data?.faculty || result?.data || [];
      setFaculty(
        (Array.isArray(rows) ? rows : []).map((member, index) => ({
          ...member,
          id: member?.id || member?._id || `staff-${index}-${Date.now()}`,
      ...
      showPopup("Error", getApiErrorMessage(error, "Unable to load staff"));
      setFaculty([]);
    }
  };
  ...
  const allDepartmentSectionOptions = useMemo(() => {
    ...
    batchDepartmentSections.forEach((item) => {
      ...
      if (batch && dept && sec)
        unique.add(`${normalize(batch)}|${normalize(dept)}|${normalize(sec)}`);
    });
  ...
  const assignedDepartmentSectionsSize = useMemo(() => {
    ...
    faculty.forEach((member) => {
      getFlattenedAllowdept(member.allowdept).forEach((item) =>
        assigned.add(
          `${normalize(item.batch)}|${normalize(item.dept)}|${normalize(item.sec)}`,
        ),
      );
    });
  ...
  const globalAvailableClassesCount = Math.max(
    0,
    allDepartmentSectionOptions.length - assignedDepartmentSectionsSize,
  );
  ...
  const availableGroupedOptions = useMemo(() => {
    ...
    batchDepartmentSections.forEach((item) => {
      ...
      const isSelected = flatFormSelections.some(
        (s) =>
          normalize(s.batch) === normalize(batch) &&
          normalize(s.dept) === normalize(dept) &&
          normalize(s.sec) === normalize(sec),
      );

      if (!isSelected) {
        if (!groups[batch]) groups[batch] = [];
        const exists = groups[batch].some(
          (c) => c.dept === dept && c.sec === sec,
        );
    ...
    return Object.keys(groups)
      ...
      .map((batch) => ({
        batch,
        classes: groups[batch].sort(
          (a, b) => a.dept.localeCompare(b.dept) || a.sec.localeCompare(b.sec),
        ),
      }));
  ...
  const filteredFaculty = useMemo(() => {
    ...
    return faculty.filter((member) => {
      ...
      return [
        ...
        member?.email,
        member?.phoneNo,
        ...flatDepts.map((a) => `${a.batch} ${a.dept} ${a.sec}`),
      ]
        .filter(Boolean)
  ...
  const removeDepartmentSection = (batch, dept, sec) => {
    setForm((prev) => {
      const newAllowdept = prev.allowdept
        .map((batchGroup) => {
          if (normalize(batchGroup.batch) === normalize(batch)) {
            return {
              ...batchGroup,
              classes: batchGroup.classes.filter(
                (c) =>
                  !(
                    normalize(c.dept) === normalize(dept) &&
                    normalize(c.sec) === normalize(sec)
                  ),
              ),
            };
          }
  ...
  const toggleDepartmentSection = (batch, dept, sec) => {
    setForm((prev) => {
      const newAllowdept = prev.allowdept.map((batchGroup) => ({
        ...batchGroup,
        classes: [...batchGroup.classes],
      }));
      const batchIndex = newAllowdept.findIndex(
        (b) => normalize(b.batch) === normalize(batch),
      );

      if (batchIndex >= 0) {
        const clsIndex = newAllowdept[batchIndex].classes.findIndex(
          (c) =>
            normalize(c.dept) === normalize(dept) &&
            normalize(c.sec) === normalize(sec),
        );
        if (clsIndex >= 0) {
          ...
          if (newAllowdept[batchIndex].classes.length === 0)
            newAllowdept.splice(batchIndex, 1);
  ...
  const handlePhotoChange = async (event) => {
    ...
    if (!file.type.startsWith("image/"))
      return showPopup("error", "Please select a valid image");
    if (file.size >= MAX_ORIGINAL_IMAGE_SIZE)
      return showPopup("error", "Photo must be less than 1MB");
    try {
      ...
      showPopup("error", getApiErrorMessage(error, "Unable to compress photo"));
    } finally {
      event.target.value = "";
    }
  };
  ...
  const validateForm = () => {
    if (!form.name.trim())
      return showPopup("error", "Staff name is required") && false;
    if (!form.email.trim())
      return showPopup("error", "Email is required") && false;
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email.trim()))
      return showPopup("error", "Enter a valid email address") && false;
    if (!/^[0-9]{10}$/.test(form.phoneNo))
      return showPopup("error", "Enter a valid 10 digit phone number") && false;
    if (flatFormSelections.length === 0)
      return showPopup("error", "Please select at least one class") && false;
    if (!form.semester.trim())
      return showPopup("error", "Semester is required") && false;
    return true;
  };
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const staffData = {
        ...
        role: "staff",
        photo: form.photo || editingFaculty?.photo || "",
      };
      const payload = {
        operation: isEditing ? "update" : "insert",
        data: { ...(isEditing && id ? { id, _id: id } : {}), ...staffData },
      };
      ...
      showPopup(
        ...
        error?.response?.status === 413
          ? "Photo is too large."
          : getApiErrorMessage(error, "Unable to save staff"),
      );
    } finally {
      setSaving(false);
    }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (result?.success === false)
        throw new Error(result?.message || "Unable to delete staff");
      await fetchStaff();
      ...
      showPopup("error", getApiErrorMessage(error, "Unable to delete staff"));
    } finally {
      setSaving(false);
    }
  };
  ...
  const FacultyCard = ({ member }) => {
    ...
    const firstBatch =
      groupedAssignments.length > 0 ? groupedAssignments[0] : null;
      ...
      <article className="faculty-card">
        ...
        <div className="faculty-card-content">
              ...
              <div className="batch-assignment-group card-display-group">
                ...
                <div className="batch-assignment-body card-display-body">
                  {displayClasses.map((cls, cIdx) => (
                    <div
                      className="dept-sec-chip display-only"
                      key={`card-${firstBatch.batch}-${cls.dept}-${cls.sec}-${cIdx}`}
                    >
                  ...
                  {hiddenCount > 0 && (
                    <div className="more-classes-indicator">
                      +{hiddenCount} more
                    </div>
                  )}
                </div>
          ...
          <button
            ...
            onClick={() => setViewDetailsModal(member)}
          >
    ...
    <div className="faculty-page">
        ...
        <div className="faculty-header-left">
          ...
          <div>
            ...
            <p className="faculty-subtitle">
              Manage faculty members and their assigned sections.
            </p>
          </div>
        </div>
        <button
      ...
      <section className="faculty-summary">
        ...
        <div className="summary-card">
          ...
          <div>
            ...
            <strong>{assignedDepartmentSectionsSize}</strong>
          </div>
        </div>
        <div className="summary-card">
          <div className="summary-icon">
        ...
        <div className="summary-card">
          ...
          <div>
            ...
            <strong>{globalAvailableClassesCount}</strong>
          </div>
        </div>
      </section>
      ...
      <div className="faculty-toolbar">
        <div className="faculty-search">
          ...
          <input
            type="text"
            placeholder="Search faculty, batch, branch or section..."
            value={search}
      ...
      <section className="faculty-section-group">
        <div className="group-heading">
          <div>
            <span className="group-label">ACADEMIC TEAM</span>
            <h2>Staff Members</h2>
          </div>
              ...
              <div className="view-profile-info">
                <div className="view-profile-photo">
                  {viewDetailsModal.photo ? (
                    <img src={viewDetailsModal.photo} alt="Faculty" />
                  ) : (
                    <UserRound size={32} />
                  )}
                </div>
                <div className="view-profile-details">
                  <div className="vpd-item">
                    <Mail size={14} /> {viewDetailsModal.email}
                  </div>
                  <div className="vpd-item">
                    <Phone size={14} /> {viewDetailsModal.phoneNo}
                  </div>
                  <div className="vpd-item">
                    <GraduationCap size={14} /> Semester{" "}
                    {viewDetailsModal.semester}
                  </div>
                </div>
              </div>
                ...
                <div className="selected-assignments form-selected-assignments-scroll">
                  {getGroupedAllowdept(viewDetailsModal).length > 0 ? (
                    getGroupedAllowdept(viewDetailsModal).map((group, idx) => (
                      <div
                        className="batch-assignment-group"
                        key={`modal-${group.batch}-${idx}`}
                      >
                        ...
                        <div className="batch-assignment-body">
                          {group.classes.map((cls, cIdx) => (
                            <div
                              className="dept-sec-chip display-only"
                              key={`modal-${group.batch}-${cls.dept}-${cls.sec}-${cIdx}`}
                            >
                              <div className="dept-sec-chip-text">
                                {cls.dept} <span>• Sec {cls.sec}</span>
                              </div>
                    ...
                    <div className="assignment-empty-state">
                      ...
                      <div>
                        ...
                        <span>
                          This staff member currently has no active assignments.
                        </span>
                      </div>
                    </div>
                  )}
        ...
        >
          <div className="faculty-modal">
            ...
            <form
              ...
              className="form-con  form-selected-assignments-scroll"
            >
              <div className="modal-body">
                  ...
                  <div className="form-group">
                    ...
                    <div className="static-role-field">
                      ...
                      <div className="static-role-content">
                        <span className="static-role-value">Staff</span>
                      </div>
                    </div>
                  </div>
                ...
                <div
                  className="form-group faculty-assignment-group"
                  style={{ marginTop: "6px" }}
                >
                  ...
                  <div className="custom-multi-select" ref={dropdownRef}>
                    ...
                    >
                      <div className="trigger-text">
                        ...
                        {loadingScheduleData
                          ? "Loading..."
                          : `Select Classes (${flatFormSelections.length} Selected)`}
                      </div>
                          ...
                          availableGroupedOptions.map((group) => (
                            <div key={group.batch} className="select-group">
                              ...
                              {group.classes.map((cls) => (
                                <div
                                  key={`${group.batch}-${cls.dept}-${cls.sec}`}
                  ...
                  {form.allowdept.length > 0 ? (
                    <div className="selected-assignments">
                      {form.allowdept.map((batchGroup, bIdx) => (
                        <div
                          className="batch-assignment-group"
                          key={`form-${batchGroup.batch}-${bIdx}`}
                        >
                          <div className="batch-assignment-header">
                            <GraduationCap size={15} /> Batch:{" "}
                            {batchGroup.batch}
                          </div>
                          <div className="batch-assignment-body">
                            {batchGroup.classes.map((cls, cIdx) => (
                              <div
                                className="dept-sec-chip"
                                key={`form-${batchGroup.batch}-${cls.dept}-${cls.sec}-${cIdx}`}
                              >
                                <div className="dept-sec-chip-text">
                                  {cls.dept} <span>• Sec {cls.sec}</span>
                                </div>
                                <button
                                  type="button"
                                  className="dept-sec-chip-remove"
                                  onClick={(e) => {
                                    e.stopPropagation();
                                  ...
                                  disabled={saving}
                                  title={`Remove ${cls.dept} Section ${cls.sec}`}
                                >
                                  <X size={13} />
                                </button>
                              </div>
                    ...
                    <div className="assignment-empty-state">
                      ...
                      <div>
                        <strong>No classes assigned</strong>
                        <span>
                          Use the dropdown above to assign a branch & section.
                        </span>
                      </div>
                    </div>
                  )}
